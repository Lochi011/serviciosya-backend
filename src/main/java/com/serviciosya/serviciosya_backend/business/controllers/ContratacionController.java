...
public class ContratacionController {
    ...
    @GetMapping("/terminadas-demandante")
    public ResponseEntity<?> listarContratacionesTerminadasDemandante(@RequestHeader("Authorization") String token) {
        if (token == null || !token.startsWith("Bearer ")) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Authorization token is missing or invalid");
        }
        String jwtToken = token.substring(7);
        String emailDemandante = jwtService.getUsernameFromToken(jwtToken);
        try {
            return ResponseEntity.ok(contratacionMgr.obtenerContratacionesTerminadasPorDemandante(emailDemandante));
        } catch (EntidadNoExiste e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e.getMessage());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error interno al listar las contrataciones del demandante.");
    ...
    @GetMapping("/aceptadas-ofertante")
    public ResponseEntity<?> listarContratacionesAceptadasOfertante(@RequestHeader("Authorization") String token) {
        if (token == null || !token.startsWith("Bearer ")) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Authorization token is missing or invalid");
        }
        String jwtToken = token.substring(7);
        String emailOfertante = jwtService.getUsernameFromToken(jwtToken);
        try {
            return ResponseEntity.ok(contratacionMgr.obtenerContratacionesAceptadasPorOfertante(emailOfertante));
        } catch (EntidadNoExiste e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e.getMessage());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error interno al listar las contrataciones del ofertante.");
